...
func (h *StandoffPublishHandler) Handle(ctx context.Context, data []byte) error {
	...
	if err := json.Unmarshal(data, &job); err != nil {
		h.log.Error("failed to unmarshal PublishStandoffsJob", "error", err)
		return err
	}

	thresholdTime := job.GetThresholdTime()
	h.log.Info("publishing standoff reviews older than", "time", thresholdTime)

	publishedCount, err := h.reviewSvc.PublishExpiredStandoffs(ctx, thresholdTime)
	if err != nil {
		h.log.Error("failed to publish expired standoffs", "error", err)
		return err
	}

	h.log.Info("successfully published expired standoff reviews", "count", publishedCount)
	return nil
}
