...
func (s *PayoutServiceImpl) recordCommissionInternal(
	...
	if err := walletRepo.UpdateBalance(ctx, escrowWalletID, -amount); err != nil {
		return nil, fmt.Errorf("failed to update escrow wallet: %w", err)
	}

	// Update platform wallet balance (credit)
	if err := walletRepo.UpdateBalance(ctx, platformWalletID, amount); err != nil {
...
func (s *PayoutServiceImpl) recordPayoutInternal(
	...
	if err := walletRepo.UpdateBalance(ctx, hostWalletID, amount); err != nil {
